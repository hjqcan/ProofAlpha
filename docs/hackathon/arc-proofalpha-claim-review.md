...
| A subscribed wallet can unlock signal details. | Yes | `subscription.json`, `entitlement-status.json`, `access-allowed.json`, unlocked screenshot. | Default run uses local Hardhat EVM/testnet-style evidence. |
...
| Revenue settlement path records splits. | Yes | `revenue-settlement.json`, `revenue-settlement-cli-journal.json`, revenue screenshot. | Settlement evidence only; no production accounting or tax claim. |
...
- Say "revenue settlement record," not "production payout."
